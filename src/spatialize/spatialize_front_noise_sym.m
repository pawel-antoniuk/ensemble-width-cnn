...
params.FinalResultsOutputDir = '../../data/spatialize/spatresults';
...
params.SymmetricalWidths = [0 5 10 15 20 25 30 35 40 45];
...
start = tic;
parfor iRep = 1:params.NRepetitions     
...
function [outSpatResults,outSpatMetaResults] = spatializeSong(HRTFs, tracks, trackNames, audioName, params)    
    ...
    for comb = allcomb(...
        ...
        metaResults = getSceneMetaresultSymmetrical(HRTFs(iHRTF), audioName, ...
            trackNames, symmetricalWidth, params);          
        
        spatResults = spatializeAudioTracks(...
            tracks, HRTFs(iHRTF), metaResults, params);

        outSpatResults(iSymmetricalWidth, iHRTF,:,:,:) = spatResults;
        outSpatMetaResults(iSymmetricalWidth, iHRTF) = {reshape(metaResults, 1, 1, [])};
        
        fprintf('Progress [w %d/%d] [HRTF %d/%d]\n', ...
            iSymmetricalWidth, ...
...
function metaResults = getSceneMetaresultSymmetrical(HRTF, audioName, trackNames, symmetricalWidth, params)
    ...
    metaResults.AzimuthEnsembleOffset = 0;
end
...
function spatResults = saveSpatResults(spatResults, spatMetaresults, ...
    ...
    for comb = allcomb(...
        ...
        [iSymmetricalWidth,iHRTF] = cComb{:};
        symmetricalWidth = params.SymmetricalWidths(iSymmetricalWidth);        
        spatResult = squeeze(spatResults(iSymmetricalWidth, iHRTF, :, :));
        spatMeta = spatMetaresults(iSymmetricalWidth, iHRTF);
