...
data_dir = Path("/app/data/spatialize/spatresults-location/spat")
model = Model(request).try_load_or_compute_input_data()
errors_width = []
errors_location = []

spectrogram_per_actual_location_l = {}
spectrogram_per_actual_location_r = {}
...
for sample_path in list(data_dir.glob("*.wav")):
    ...
    actual_location = int(metadata.actual_location) // 5 * 5
    spectrogram_per_actual_location_l.setdefault(actual_location, []).append(
        features["in_mag"][0, :, :, 0]
    )
    spectrogram_per_actual_location_r.setdefault(actual_location, []).append(
        features["in_mag"][0, :, :, 1]
    )
...
locations = [-30, -15, 0, 15]
nrows = len(locations)
ncols = 4

ref_left = np.mean(spectrogram_per_actual_location_l[0], axis=0)
ref_right = np.mean(spectrogram_per_actual_location_r[0], axis=0)
...
for i, actual_location in enumerate(locations):
    left = np.abs(
        np.mean(spectrogram_per_actual_location_l[actual_location], axis=0) - ref_left
    )
    right = np.abs(
        np.mean(spectrogram_per_actual_location_r[actual_location], axis=0) - ref_right
    )
    diff = np.abs(np.abs(left - right) - ref_diff)

    plt.subplot(nrows, ncols, i * ncols + 1, polar=True)
    plt.text(np.pi / 4, 1.1, f"θ = {actual_location}°\nω = 60°")
    actual_location_rad = actual_location / 180 * np.pi
    actual_width_rad = 60 / 180 * np.pi
    theta = np.linspace(
    ...
    plt.imshow(left, vmin=0, vmax=1, **pltargs)
    labels()

    plt.subplot(nrows, ncols, i * ncols + 3)
    plt.imshow(right, vmin=0, vmax=1, **pltargs)
    labels()

    plt.subplot(nrows, ncols, i * ncols + 4)
    plt.imshow(diff, vmin=0, vmax=1, **pltargs)
    labels()
    # plt.subplot(nrows, ncols, i * ncols + 4)
    # plt.colorbar()
...
plt.tight_layout()
plt.show()
plt.savefig(f"/app/data/analyze/figure/samples_location.png")
